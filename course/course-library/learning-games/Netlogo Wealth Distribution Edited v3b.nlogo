...
[
  ...
  num-people   ; total number of people
  percent-best-land   ; percentage of land with the maximum amount of grain
  grain-growth-interval   ; the frequency at which grain is grown
  num-grain-grown   ; the amount of grain grown on each patch
  gini-index-reserve
  lorenz-points
]
...
to set-initial-turtle-vars
  ...
  set metabolism metabolism-min +
    random(metabolism-max - metabolism-min + 1)

  ;; If the INITIAL-WEALTH-EQUAL switch is set to true,
  ;; set the wealth equal to the value specified by the
  ;; INITIAL-WEALTH-LEVEL slider; otherwise, assign a
  ;; random initial wealth level
  ifelse initial-wealth-equal = true
    [ set wealth initial-wealth-level ]
    [ set wealth metabolism + random 50 ]

  set vision min-vision +
    random(max-vision - min-vision + 1)
end
...
to move-eat-age-die  ;; turtle procedure
  ...
  ;; holds, then you "die" and are "reborn"; in the original version,
  ;; your variables are just reset to new random values; in this
  ;; version, if the INHERITED-WEALTH slider is set to true, the
  ;; "reborn" turtle inherits the wealth of the previous turtle
  if (wealth < 0) or (age >= life-expectancy)
  [
    ifelse inherited-wealth = true
    [
      ;; Store this turtle's wealth as the "inheritance"; if this
      ;; value is less than zero, set the inheritance to zero
      let inheritance wealth
      if inheritance < 0 [set inheritance 0]

      ;; Generate new values
      set-initial-turtle-vars

      ;; Add the inheritance to the turle's initial wealth value
...
LIFE-EXPECTANCY-MIN is the shortest number of ticks that a person can possibly live.  LIFE-EXPECTANCY-MAX is the longest number of ticks that a person can possibly live.  The METABOLISM-MIN and METABOLISM-MAX sliders set the lowest and highest possible amounts of grain, respectively, that a person could eat per clock tick.  The MIN-VISION and MAX-VISION sliders are the nearest and farthest possible distances, respectively, that any person could see. If the INITIAL-WEALTH-EQUAL switch is set to "on," then each person is initially given the level of wealth chosen by the INITIAL-WEALTH-LEVEL slider. If this slider is set to "off," then the initial wealth level is the person's metabolism value plus a random value between 0 and 49. Finally, if the INHERITED-WEALTH switch is set to "on," each offspring inherents the wealth of its parent in addition to the wealth it would otherwise have been given (if a parent has negative wealth when it dies, then the inheritance is zero).
...
Copyright 1998 Uri Wilensky. (Modified by Stuart Iler.)
